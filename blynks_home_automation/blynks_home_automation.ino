...
byte colPins[COLS] = { 32, 34, 38, 40 };  //connect to the column pinouts of the keypad
...
void setSocketSchedule() {
  lcd.clear();
  lcd.print("Set ON Time (HHMM):");
  lcd.setCursor(0, 1);
  ...
  onTime = on.substring(0, 2) + ":" + on.substring(2, 4);

  lcd.clear();
  lcd.print("Set OFF Time (HHMM):");
  lcd.setCursor(0, 1);
  ...
  offTime = off.substring(0, 2) + ":" + off.substring(2, 4);

  lcd.clear();
  lcd.print("ON: " + onTime);
  lcd.setCursor(0, 1);
  lcd.print("OFF: " + offTime);
  delay(2000);
}

void handleSocketTimer() {
